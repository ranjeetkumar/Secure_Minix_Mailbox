...
// Updated for Project 2. Added mailbox messages
typedef struct {
	uid_t caller_uid;
	uint32_t mboxd;
	uint32_t mailbox_type;
	uint8_t padding[44];
} mess_mailbox_create;
_ASSERT_MSG_SIZE(mess_mailbox_create);
...
_ASSERT_MSG_SIZE(mess_mailbox_remove);
...
_ASSERT_MSG_SIZE(mess_mail_deposit);
...
_ASSERT_MSG_SIZE(mess_mail_retrieve);

// Updated for Project 3. Added mailbox security messages
typedef struct {
	uid_t caller_uid;
	uint32_t mboxd;
	uid_t uid;
	uint8_t padding[44];
} mess_mailbox_update_acl;
_ASSERT_MSG_SIZE(mess_mailbox_update_acl);
...
typedef struct {
	...
	union {
		...
		// Updated for Project 2. Added union members for mailbox messages
		...
		// Updated for Project 3. Added union members for mailbox security messages
		mess_mailbox_update_acl m_mailbox_update_acl;
