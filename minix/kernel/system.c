...
// } Updated for Project 2. Allow messages from user processes to kernel
...
void system_init(void)
{
  ...
  // Updated for Project 2. Added mappings for new kernel calls in call table
  map(SYS_CREATE_MAILBOX, do_create_mailbox);	/* create mailbox */
  map(SYS_REMOVE_MAILBOX, do_remove_mailbox);	/* remove mailbox */
  map(SYS_DEPOSIT_MAIL, do_deposit_mail);	/* deposit mail */
  map(SYS_RETRIEVE_MAIL, do_retrieve_mail);	/* retrieve mail */
  // Updated for Project 3. Added mappings for new security kernel calls in call table
  map(SYS_ADD_OWNER, do_add_owner);	/* add owner */
  map(SYS_REMOVE_OWNER, do_remove_owner);	/* remove owner */
  map(SYS_ADD_MAILBOX_SENDER, do_add_mailbox_sender);	/* add mailbox sender */
  map(SYS_REMOVE_MAILBOX_SENDER, do_remove_mailbox_sender);	/* remove mailbox sender */
  map(SYS_ADD_MAILBOX_RECEIVER, do_add_mailbox_receiver);	/* add mailbox receiver */
  map(SYS_REMOVE_MAILBOX_RECEIVER, do_remove_mailbox_receiver);	/* remove mailbox receiver */
